...
pub fn main() {
    ...
    if common::load_data("../data/07.txt", &mut data).is_ok() {
        ...
        eprintln!("cannot open ../data/07.txt");
        std::process::exit(1);
    }
}
...
fn count_contents(graph: &GraphMap<&str, f32, Directed>, outer: &str) -> f32 {
    graph
        ...
        .map(|inner| {
            count_contents(graph, inner) * *graph.edge_weight(outer, inner).unwrap_or(&0.0)
        })
        .sum::<f32>()
        + 1.0
}
...
mod tests {
    ...
    fn test_part_1() {
        ...
        if common::load_data("../data/07.txt", &mut data).is_ok() {
            ...
            assert_eq!(part_1(&data_as_strs), 287)
        } else {
            eprintln!("cannot open ../data/07.txt");
            std::process::exit(1);
        }
    ...
    fn test_part_2() {
        ...
        if common::load_data("../data/07.txt", &mut data).is_ok() {
            let data_as_strs: Vec<&str> = data.iter().map(|v| v.as_str()).collect();
            assert_eq!(part_2(&data_as_strs), 48_160)
        } else {
            eprintln!("cannot open ../data/07.txt");
            std::process::exit(1);
        }
