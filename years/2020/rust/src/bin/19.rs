...
pub fn main() {
    ...
    if common::load_data("../data/19.txt", &mut data).is_ok() {
        ...
        eprintln!("cannot open ../data/19.txt");
        std::process::exit(1);
    }
}
    ...
    match rule {
        ...
        Rule::Or(lhs, rhs) => {
            if let Some(match_result) =
                match_rule(rules, &Rule::And(Vec::from(lhs.as_slice())), text)
            {
                Some(match_result)
            } else {
...
mod tests {
    ...
    fn test_part_1() {
...

        {
            ...
            if common::load_data("../data/19.txt", &mut data).is_ok() {
                ...
                assert_eq!(part_1(&data_as_strs), 198)
            } else {
                eprintln!("cannot open ../data/19.txt");
                std::process::exit(1);
            }
    ...
    fn test_part_2() {
...

        {
            ...
            if common::load_data("../data/19.txt", &mut data).is_ok() {
                let data_as_strs: Vec<&str> = data.iter().map(|v| v.as_str()).collect();
                assert_eq!(part_2(&data_as_strs), 0)
            } else {
                eprintln!("cannot open ../data/19.txt");
                std::process::exit(1);
            }
