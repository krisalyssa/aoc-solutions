...
pub fn main() {
    ...
    if common::load_data("../data/10.txt", &mut data).is_ok() {
        ...
        eprintln!("cannot open ../data/10.txt");
        std::process::exit(1);
    }
}
...
mod tests {
    ...
    fn test_part_1() {
        ...
        if common::load_data("../data/10.txt", &mut data).is_ok() {
            ...
            assert_eq!(part_1(&data_as_strs), 1856)
        } else {
            eprintln!("cannot open ../data/10.txt");
            std::process::exit(1);
        }
    ...
    fn test_part_2() {
        ...
        if common::load_data("../data/10.txt", &mut data).is_ok() {
            let data_as_strs: Vec<&str> = data.iter().map(|v| v.as_str()).collect();
            assert_eq!(part_2(&data_as_strs), 2_314_037_239_808)
        } else {
            eprintln!("cannot open ../data/10.txt");
            std::process::exit(1);
        }
    ...
    fn test_adapters() {
        ...
        }
        {
            ...
            assert_eq!(
                adapters(&data),
                vec![
                    1, 2, 3, 4, 7, 8, 9, 10, 11, 14, 17, 18, 19, 20, 23, 24, 25, 28, 31, 32, 33,
                    34, 35, 38, 39, 42, 45, 46, 47, 48, 49,
                ]
            );
        }
    }
    ...
    fn test_differences() {
        ...
        }
        {
            ...
            assert_eq!(
                differences(&adapters),
                vec![
                    1, 1, 1, 1, 3, 1, 1, 1, 1, 3, 3, 1, 1, 1, 3, 1, 1, 3, 3, 1, 1, 1, 1, 3, 1, 3,
                    3, 1, 1, 1, 1, 3
                ]
            );
        }
