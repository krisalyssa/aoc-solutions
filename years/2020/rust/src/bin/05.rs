...
pub fn main() {
    ...
    if common::load_data("../data/05.txt", &mut data).is_ok() {
        ...
        eprintln!("cannot open ../data/05.txt");
        std::process::exit(1);
    }
}
...
fn binary_search(r: &Range<u32>, sequence: &str, bottom: char, top: char) -> u32 {
    ...
    if sequence.len() == 1 {
        ...
        match c {
            _ if c == bottom => {
                binary_search(&(r.start..midpoint - 1), &sequence[1..], bottom, top)
            }
...
mod tests {
    ...
    fn test_part_1() {
        ...
        if common::load_data("../data/05.txt", &mut data).is_ok() {
            ...
            assert_eq!(part_1(&data_as_strs), 930)
        } else {
            eprintln!("cannot open ../data/05.txt");
            std::process::exit(1);
        }
    ...
    fn test_part_2() {
        ...
        if common::load_data("../data/05.txt", &mut data).is_ok() {
            let data_as_strs: Vec<&str> = data.iter().map(|v| v.as_str()).collect();
            assert_eq!(part_2(&data_as_strs), 515)
        } else {
            eprintln!("cannot open ../data/05.txt");
            std::process::exit(1);
        }
