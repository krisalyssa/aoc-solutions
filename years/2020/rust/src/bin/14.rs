...
pub fn main() {
    ...
    if common::load_data("../data/14.txt", &mut data).is_ok() {
        ...
        eprintln!("cannot open ../data/14.txt");
        std::process::exit(1);
    }
}
...
mod tests {
    ...
    fn test_part_1() {
        ...
        if common::load_data("../data/14.txt", &mut data).is_ok() {
            ...
            assert_eq!(part_1(&data_as_strs), 7_997_531_787_333)
        } else {
            eprintln!("cannot open ../data/14.txt");
            std::process::exit(1);
        }
    ...
    fn test_part_2() {
        ...
        if common::load_data("../data/14.txt", &mut data).is_ok() {
            let data_as_strs: Vec<&str> = data.iter().map(|v| v.as_str()).collect();
            assert_eq!(part_2(&data_as_strs), 3_564_822_193_820)
        } else {
            eprintln!("cannot open ../data/14.txt");
            std::process::exit(1);
        }
